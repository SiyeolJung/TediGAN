...
def parse_args():
  ...
  parser.add_argument("--ckpt", type=str, default="pretrained_models/network-snapshot-009354.pt",
...
def main(args):
    ...
    text_inputs = torch.cat([clip.tokenize(args.description,truncate=True)]).cuda()
    ...
    if args.mode =="man":
        ...
        z_init_not_trunc = torch.randn(512, 512).cuda()
        with torch.no_grad():
    ...
    z = z_mean.detach().clone().repeat(1, 14, 1) # original = 1, 18, 1
...
if __name__ == "__main__":
    ...
    torchvision.utils.save_image(result_image.detach().cpu(), os.path.join(
        args.results_dir, "final_result6.png"), normalize=True, scale_each=True, range=(-1, 1))
